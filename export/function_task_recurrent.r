...
library(tidyr)
library(readr)
source('./_function_recurrent_expand_name.r')
...
eps = read_csv('../results/function_task_recurrent_mse_expectation.csv') %>%
...
dat.last = dat %>%
  ...
  summarise(
    ...
    range.extrapolation.last = metric.test.range_extrapolation[best.model.step],
    ...
    range.extrapolation.step.solved = first.solved.step(step, metric.test.range_extrapolation, threshold),
    sparse.error.max = sparse.error.max[best.model.step],
    solved = replace_na(metric.test.extrapolation[best.model.step] < threshold, FALSE),
    range.solved = replace_na(metric.test.range_extrapolation[best.model.step] < threshold, FALSE),
    model = last(model),
...
save.table(
  dat.last.rate %>% filter(
    (operation %in% c('$+$', '$-$') & model %in% c('Linear', 'NAU', '$\\mathrm{NAC}_{+}$', 'NALU', 'MCLSTM')) |
    (operation %in% c('$\\times$') & model %in% c('NMU', '$\\mathrm{NAC}_{\\bullet}$', 'NALU', 'MULMCFC'))
  ),
...
save.table(
  dat.last.rate %>% filter(
    (operation %in% c('$+$', '$-$') & model %in% c('Linear', '$\\mathrm{NAC}_{+}$', 'NALU')) |
